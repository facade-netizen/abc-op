...
class CreditLimitLogScreen extends StatefulWidget {
  const CreditLimitLogScreen({super.key, required this.title});
  final String title;
  @override
  State<CreditLimitLogScreen> createState() => _CreditLimitLogScreenState();
}

class _CreditLimitLogScreenState extends State<CreditLimitLogScreen> {
  ...
  int currentPage = 1;
  int totalPages = 1;
  final Map<int, List<CreditLimitLogModel>> _pageCache = <int, List<CreditLimitLogModel>>{};

  DateTime get now => DateTime.now();

  void fetchCreditLimitLogs({DateTime? fromDate, DateTime? toDate, int page = 1}) {
    ...
    context.read<FetchCreditLimitLogsBloc>().add(FetchCreditLimitLogs(body: requestBody));
  }
  ...
  void fetchPage(int page) {
    ...
    fetchCreditLimitLogs(page: safePage);
  }
  ...
  Widget buildQuickFilterButton(String title, DateFilterType type) {
    return CustomOCTAButton(
      ...
      action: () {
        ...
        currentPage = 1;
        fetchCreditLimitLogs(fromDate: range.from, toDate: range.to, page: 1);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  BlocBuilder<FetchCreditLimitLogsBloc, FetchCreditLimitLogsState>(
                    builder: (context, als) {
                      List<CreditLimitLogModel> changePassLogs = [];
                      if (als is FetchCreditLimitLogsSuccess) {
                        changePassLogs = als.creditLimitResponse.data;
                      }
                      return DownloadReport(
                        reportName: widget.title,
                        headerTitles: creditLimitColumns.map((e) => e.label).toList(),
                        rowData: changePassLogs.map((row) {
                          return creditLimitColumns.map((col) {
                            if (col.value != null) {
        ...
        BlocBuilder<FetchCreditLimitLogsBloc, FetchCreditLimitLogsState>(
          builder: (context, als) {
            if (als is FetchCreditLimitLogsProgress) {
              return LoaderContainerWithMessage();
            }

            if (als is FetchCreditLimitLogsSuccess) {
              totalPages = als.creditLimitResponse.totalPages;
              _pageCache[als.creditLimitResponse.page] = als.creditLimitResponse.data;
            }

            final List<CreditLimitLogModel> changePassLogs = _pageCache[currentPage] ?? [];

            return CustomPaginatedTable<CreditLimitLogModel>(
              topPadding: 5,
              data: changePassLogs,
