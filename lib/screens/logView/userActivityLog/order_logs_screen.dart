...
class UserOrderLogScreen extends StatefulWidget {
  const UserOrderLogScreen({super.key, required this.title});
  final String title;
  @override
  State<UserOrderLogScreen> createState() => _UserOrderLogScreenState();
}

class _UserOrderLogScreenState extends State<UserOrderLogScreen> {
  ...
  void fetchActivityLogs({DateTime? fromDate, DateTime? toDate, int page = 1}) {
    ...
    context.read<FetchUserActivityLogsBloc>().add(
      FetchUserActivityLogs(userId: userIdController.text, from: from, to: to, page: currentPage, limit: limit, logType: UserActivityLogType.orderActivity),
    );
  }
                  ...
                  BlocBuilder<FetchUserActivityLogsBloc, FetchUserActivityLogsState>(
                    builder: (context, als) {
                      ...
                      return DownloadReport(
                        ...
                        headerTitles: orderLogColumns.map((e) => e.label).toList(),
                        rowData: activityLogs.map((row) {
                          return orderLogColumns.map((col) {
                            if (col.value != null) {
