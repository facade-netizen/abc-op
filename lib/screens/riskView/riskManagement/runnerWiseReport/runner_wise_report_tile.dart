...
class RunnerWiseReportTile extends StatelessWidget {
  final RunnerWiseReportData report;
  const RunnerWiseReportTile({
    super.key,
...
class RunnerWiseReportRow extends StatelessWidget {
  final RunnerWiseReportDetail detail;
  final int index;
  final bool isLast;

  const RunnerWiseReportRow({
    super.key,
          ...
          child: Row(
            children: [
              ...
              Expanded(
                ...
                child: Row(
                  children: [
                    RWCell(val: detail.avgLayOdds),
                    RWCell(val: detail.layStake),
                    RWCell(val: detail.liability),
                  ],
                ),
              ),
            ],
...
class RWHCell extends StatelessWidget {
  const RWHCell({super.key, required this.title});
  final String title;
  @override
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        child: HighlightText(
          title,
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: black),
        ),
      ),
    );
  }
...
class RWCell extends StatelessWidget {
  const RWCell({super.key, required this.val});
  final double val;
  @override
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        alignment: Alignment.centerRight,
        child: HighlightText(
          formattedAmounts(val),
          style: TextStyle(color: val < 0 ? red : black, fontSize: 12),
        ),
      ),
    );
  }
