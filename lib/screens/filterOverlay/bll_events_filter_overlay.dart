...
class BllEventsFilterOverlayNew extends StatefulWidget {
  final double width;
  final Function(Map<String, dynamic> values)? onSubmitted;

  const BllEventsFilterOverlayNew({
    super.key,
    required this.width,
    this.onSubmitted,
  });
  ...
  @override
  State<BllEventsFilterOverlayNew> createState() => _BllEventsFilterOverlayNewState();
}

class _BllEventsFilterOverlayNewState extends State<BllEventsFilterOverlayNew> {
  ...
  Widget build(BuildContext context) {
    return FilterOverlay(
      ...
      body: BlocBuilder<FetchBetListLiveEventsBloc, FetchBetListLiveEventsState>(
        builder: (context, fbl) {
          List<OrderEventData> eventList = [];
          if (fbl is FetchBetListLiveEventsSuccess) {
...
class EventRow extends StatefulWidget {
  const EventRow({
    ...
    this.sportName = '',
    this.onChanged,
    this.isSelectedForMarkets = false,
    this.onTap,
  });
      ...
      child: InkWell(
        ...
        child: Row(
          children: [
            ...
            if (widget.sportName.isNotEmpty) ...[
              HighlightText(widget.sportName),
              SizedBox(width: 4),
              Icon(
                Icons.arrow_right,
                color: Colors.grey,
